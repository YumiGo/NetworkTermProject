...
public class Handler extends Thread {
	...
	private ArrayList<Handler> allUserList; // 전체사용자
	private ArrayList<Handler> waitUserList; // 대기실사용자

	private DTO dto;
	private String online;
	String time = getAccessTime();
	private String ip;
	private String port;

	public Handler(Socket socket, ArrayList<Handler> allUserList, ArrayList<Handler> waitUserList, Connection conn)
			throws IOException {
	...
	public Handler() {//이건 다른 클래스에서 이용할 때
			...
			while (true) {
				...
				line = in.nextLine().split("\\|");
					...
					/* ENTERLOGIN */
						...
						boolean con = true; // 기존에 로그인되어있는지 안되어있는지 알아보는 변수
						...
						for (int i = 0; i < waitUserList.size(); i++) {
							if ((waitUserList.get(i).dto.getId()).compareTo(userContent[0]) == 0) {
								con = false;//로그인 되어 있으면 con false
							}
						}
						if (con) {
							int count =0;
							...
							rs = pstmt.executeQuery(sql);
							String pw="";
							while (rs.next()) {//db에 해당 id가 존재하면
								pw = rs.getString(1);
								count++;//count 증가
							}
							
							if(userContent[1].compareTo(pw)==0) {//pw가 존재
							...
							} else if (count == 0) {//id가 일치하는 행이 db에 없음 
					...
					/* Log out */
					else if (line[0].compareTo(Protocol.EXITWAITROOM) == 0) {
					
						// 나간시간 측정
						updateLogOutTime(dto);	
						String sql = "update user set online= 0,recent=? " + "where nName = '" + line[1] + "'";// 해당 닉네임의 행에
						pstmt = conn.prepareStatement(sql);
						pstmt.setString(1, dto.getTime());
						int su = pstmt.executeUpdate();
					...
					/* ADD FRIEND */
					else if (line[0].compareTo(Protocol.ADDFRIEND) == 0) {
						String userContent[] = line[1].split(",");// userContent[0]: 자기 닉네임 , userContent[1]: 친구 아이디
						String id="";
						String sql = "Select id from user where nName= '"+userContent[0]+"'";
						rs = stmt.executeQuery(sql);
						while (rs.next()) {
							id = rs.getString(1);
						}
						...
						out.println(Protocol.ADDFRIEND + "|" + friendLine);
						out.flush();
					...
					} else if (line[0].compareTo(Protocol.SEARCHFRIEND) == 0) {
						try {
							String result = "";
					...
					} else if (line[0].compareTo(Protocol.CHATREQUEST) == 0) {
					...
					} else if (line[0].compareTo(Protocol.CHATREQUEST_OK) == 0) {
						String info[] = line[1].split(",");
					...
					} else if (line[0].compareTo(Protocol.CHATREQUEST_NO) == 0) {
						String info[] = line[1].split(",");
						for (int i = 0; i < waitUserList.size(); i++) {
							if ((waitUserList.get(i).dto.getnName()).compareTo(info[0]) == 0) {// 요청받은 client에게 보냄
					...
					} else if (line[0].compareTo(Protocol.CHATEXIT) == 0) {
						String info[] = line[1].split(",");
	...
	public DTO updateLogOutTime(DTO dto) {
		time = getAccessTime();
		dto.setTime(time);
		return dto;
	}
