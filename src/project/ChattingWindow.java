...
public class ChattingWindow extends JFrame implements MouseListener, ActionListener {
	...
	private void initialize() {
		...
		lbl_yourname.setFont(new Font("함초롬돋움", Font.BOLD, 16));
		...
		scrollPane_all_opp.setViewportView(ta_all);// scrollPane에 add
		ta_all.setComponentOrientation(ComponentOrientation.LEFT_TO_RIGHT); //왼쪽정렬
		ta_all.setLineWrap(true);// 자동줄바꿈
...

	{
		...
		//ta_all.append("\n"); //반대쪽 채팅도 한줄 띄워줌.
		ta_all.setCaretPosition(ta_all.getDocument().getLength()); // 맨아래로 스크롤한다.
