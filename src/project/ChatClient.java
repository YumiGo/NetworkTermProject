...
public class ChatClient extends JFrame implements ActionListener, Runnable, ListSelectionListener, MouseListener {
					...
					if (searchCount == 0) {
						...
						frSearch.table.addMouseListener(new MouseAdapter() {
							...
							public void mousePressed(MouseEvent e) {
								...
								Object friendnName = table.getValueAt(row, 1);//nickname
								if (e.getClickCount() == 2) {
									if (friendnName.equals(userName)) {
	...
	public void JTableRefresh(String line) {
	...
	// 내가 넣은거
	public void JTableRefresh2(String line) {// 친구 검색시 테이블
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == loginUI.btnSignIn) { // 회원가입 버튼
			...
			if (!(userName.compareTo(nName) == 0)) {
				out.println(Protocol.NSEARCHCHECK + "|" + nName);
				out.flush();
			}

			if (userName.compareTo(nName) == 0 || condition_nName) {
			...
			int result = JOptionPane.showConfirmDialog(null, "탈퇴 하시겠습까?", "탈퇴", JOptionPane.YES_NO_OPTION);
	...
	public void mouseClicked(MouseEvent e) {
		...
		} else if (e.getSource() == memProc.btnD) {// 아이디 중복확인
